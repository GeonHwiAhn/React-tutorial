...
const MovieRating = () => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            // 주소값에 key 이름으로 작성된 data에서 movie라는 키값 안에 적힌 title과 이미지들을 가져올 것
            //         [    { data{ movie{ 를 가져올 것
            set영화들(res.data.data.movies); //{}중괄호들에서 data를 가지고오겠다.

            //영화데이터 모두 가지고오면 로딩중을 false로 변경하기
            set로딩(false);
            console.log(res);
        })
