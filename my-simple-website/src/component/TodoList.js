...
function TodoList() {
    ...
    const addTodo = () => {
        ...
        //      value값이 input으로 들어온 value값을 text라는 키에 저장
        //      completed = 무언가를 끝내다라는 영어
        //     할 일 추가 = 끝내야 하는 일
        ...
        //이중 배열로 배열이 한층 더 감싸져있기 때문에 값이 제대로 목록에 들어가지 않음
        setInput('');
    };
  
    // 할 일을 완료하면 완료처리를 해주는 버튼, 실수로 완료처리를 했을 경우
    // 한번 더 클릭해서 미완료처리로 변경할 수 있음
    const toggleTodo = (index) => {
      ...
      // 완료상태면 미완료 상태로, 미완료상태면 완료상태로 변경
      //   newTodos      [   index  ]     .completed    = !newTodos[index].completed;
      // 새로운목록들[사용자가선택한목록번호].완료/미완료 = 완료->미완료, 미완료->완료
      // ! 이용해서 boolean(true / false) 값을 (false / true) 값으로 변경할 수 있음
    ...
    return (
      <div className="TodoList">
        ...
        textDecoration = 밑줄
        만약에 할 일이 true이면 ling-through
              할 일이 false이면 none 이용해 줄 삭제
        ling-through = 중간에 줄 긋기
  ...
  /* 
  추후 useEffect 사용해서 웹사이트를 새로고침(F5) 하더라도 목록이 남아있을수 있도록 업데이트!
  */