...
createContext
 - 기본값을 가질 수 있고, 이 기본값은 해당 컨텍스트를 사용하는 컴포넌트(객체)가
...
Provider
 - 컨텍스트를 사용하는 컴포넌트들한테 동일한 값을 모두 제공하는 컴포넌트
   value라는 props를 받아서 하위 컴포넌트들이 접근할 수 있도록 함
...
// createContext안에 저장
// createContext안에 저장된 '부모가 전달한 값' 이
// Provider로 밑에 있는  const, function, class 등 모든 컴포넌트에 전달
// Provider로 공급자, 서비스나 제품을 제공하는 사람이나 회사
...
const 부모 = () => {
    ...
    // TextContex에 vlaue='부모가 전달한 값'을 저장하고 Provider(제공할 것)
        <TestContext.Provider value='부모가 전달한 값'>
            <자식/>
