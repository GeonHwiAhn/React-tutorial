...
const EditForm = ({user, updateUser}) => {
    // 수정하기 폼에 기존에 수정하고자하는 유저정보를 넣어주기 위해
    // userState(user) 작성
...
userList = [
    ...
    유저리스트를 item(item 말고 다른것 사용가능)으로 유저정보를 1명씩 전달받음
    전달받은 유저정보를
    user 변수명에 item에 담긴 유저1명의 정보를 저장
    저장된 use변수명을 가져와서 editFrom에 아래와 같이 작성
...
const 값변경하기 = (e) => { //값 변경에 대한 e(이벤트)가 발생하면
    const {name, value} = e. target; //이벤트가 발생한 곳에 name value 가져옴
    setFormData({...formData, [name] : value});
}
    
const 수정내용제출하기 = (e) => { //e->잠깐제출멈추기
    e.preventDefault();
    updateUser(formData);
}
    ...
    return (
        ...
        <form> {/* 폼 = submit Button = onSubmit */}
            ...
            <label>
                나이 : 
                <input type="text" name="age" value={formData.age} onChange={값변경하기} />
            </label>
            <label>
                성별 : 
